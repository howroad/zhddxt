 ...
 * @returns
 */
function checkXls(xls, modalObj) {
    if(xls==""){
    	alertAiv(modalObj, "请上传文件");
        return false;
    }else if(!/\.(xls)$/.test(xls)){
    	alertAiv(modalObj, "文件类型必须是.xls后缀");
        return false;
    }
    return true;
}
...
function alertAiv(modalObj, text) {
	modalObj.find("strong").html(text);
	modalObj.modal();
}
 ...
 * @returns
 */
function initBox(param) {
	if(param.divObj==null) { param.divObj=$(".box").eq(0); }
    if(param.hidObj==null) { param.hidObj=$("input[type='hidden']").eq(0); }
    if(param.pageObj==null) { param.pageObj=$(".pagination").eq(0); }
    var json = param.value;
	page_no = json.pageNo;
	...
	for(var i=0;i<json.list.length;i++){
		str += "<div class='row'>";
		if(json.list[i][param.data.id] == null) { json.list[i][param.data.id]=0; }
		str += "<div class='box-child col-sm-6 col-md-6' index='"+json.list[i][param.data.id]+"' style='width:350px;height:250px'><div class='thumbnail'>";
		str += "<div class='caption text-center'>";
		$.each(param.data,function(name,value) {
			if(name == "id"){ return; }
			str += "<h5>";
			if(name == "arr") {
	...
	param.divObj.html(str);
    //给tr绑定点击事件
    addTrColor(param.divObj.find(".box-child"), param.hidObj);
    //分页
    paging(page_no, total_page, param.pageObj);
    //需要初始化hid
    param.hidObj.val("");
